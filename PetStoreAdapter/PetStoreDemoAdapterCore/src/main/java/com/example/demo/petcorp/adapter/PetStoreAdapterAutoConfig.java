...
public class PetStoreAdapterAutoConfig {
  ...
  Function<String, String> newPetProcessing() {
    return adoptInfo -> {
      ...
      try {
        petAdoption = SharedConstants.MAPPER.readValue(adoptInfo, PetAdoptionDto.class);
      } catch (JsonProcessingException e) {
        SharedConstants.LOG.error("Error converting '{}' into PetAdoptionDto - {}", adoptInfo,
            e.toString());
        return null;
      }

      petAdoption.setRegistered(LocalDateTime.now(ZoneOffset.UTC));
      SharedConstants.LOG.info("Registering Pet Adoption Request - " + petAdoption);
      try {
        return SharedConstants.MAPPER.writeValueAsString(petAdoption);
      } catch (JsonProcessingException e) {
        SharedConstants.LOG.error("Error converting PetAdoptionDto '{}' into String - {}",
